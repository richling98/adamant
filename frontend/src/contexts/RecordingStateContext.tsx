...
export function RecordingStateProvider({ children }: { children: React.ReactNode }) {
  ...
  const setStatus = useCallback((status: RecordingStatus, message?: string) => {
    console.debug(`[RecordingState] Status: ${state.status} → ${status}`, message || '');
  ...
  const syncWithBackend = async () => {
    try {
      ...
      console.debug('[RecordingStateContext] Synced with backend:', backendState);
    } catch (error) {
  ...
  const startPolling = () => {
    ...
    console.debug('[RecordingStateContext] Starting state polling (500ms interval)');
  ...
  const stopPolling = () => {
    if (pollingIntervalRef.current) {
      console.debug('[RecordingStateContext] Stopping state polling');
      clearInterval(pollingIntervalRef.current);
   ...
   */
  useEffect(() => {
    console.debug('[RecordingStateContext] Setting up event listeners');
    ...
    const setupListeners = async () => {
      try {
        ...
        const unlistenStarted = await recordingService.onRecordingStarted(() => {
          console.debug('[RecordingStateContext] Recording started event');
          setState(prev => ({
            ...prev,
            isRecording: true,
            isPaused: false,
        ...
        const unlistenStopped = await recordingService.onRecordingStopped((payload) => {
          console.debug('[RecordingStateContext] Recording stopped event:', payload);
          setState(prev => {
        ...
        const unlistenPaused = await recordingService.onRecordingPaused(() => {
          console.debug('[RecordingStateContext] Recording paused event');
          setState(prev => ({
            ...prev,
            isPaused: true,
            isActive: false,
        ...
        const unlistenResumed = await recordingService.onRecordingResumed(() => {
          console.debug('[RecordingStateContext] Recording resumed event');
          setState(prev => ({
            ...prev,
            isPaused: false,
            isActive: true,
        ...
        console.debug('[RecordingStateContext] Event listeners set up successfully');
      } catch (error) {
    ...
    return () => {
      console.debug('[RecordingStateContext] Cleaning up event listeners');
   ...
   */
  useEffect(() => {
    console.debug('[RecordingStateContext] Initial mount - syncing with backend');
    syncWithBackend();
  }, []);
