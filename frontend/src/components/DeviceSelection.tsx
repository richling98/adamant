  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            className="h-8 w-8 p-0 inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors hover:bg-white/10 text-zinc-400 disabled:pointer-events-none disabled:opacity-50"
          >
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <Label htmlFor="mic-selection" className="text-sm font-medium text-zinc-300">
              Microphone
            </Label>
          </div>
          <Select
          ...
          {inputDevices.length === 0 && (
            <p className="text-xs text-zinc-500">No microphone devices found</p>
          )}

          {/* Audio Level Meters for Input Devices */}
          {showLevels && inputDevices.length > 0 && (
            <div className="space-y-2 pt-2 border-t border-white/10">
              <p className="text-xs text-zinc-400 font-medium">Microphone Levels:</p>
              {inputDevices.map((device) => {
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <Speaker className="h-4 w-4 text-zinc-400" />
            <Label htmlFor="system-selection" className="text-sm font-medium text-zinc-300">
              System Audio
            </Label>
          </div>
          ...
          {outputDevices.length === 0 && (
            <p className="text-xs text-zinc-500">No system audio devices found</p>
          )}
      ...
      <div className="text-xs text-zinc-500 space-y-1">
        <p>• <strong className="text-zinc-400">Microphone:</strong> Records your voice and ambient sound</p>
        <p>• <strong className="text-zinc-400">System Audio:</strong> Records computer audio (music, calls, etc.)</p>
        {isMonitoring && (
          <p>• <strong className="text-zinc-400">Mic Levels:</strong> Green = good, Yellow = loud, Red = too loud</p>
        )}
        {!isMonitoring && inputDevices.length > 0 && (
          <p>• <strong className="text-zinc-400">Tip:</strong> Click "Test Mic" to check if your microphone is working</p>
        )}
      </div>
    </div>
  );
