...
show_log_exit_options() {
    ...
    if docker ps --format "{{.Names}}" | grep -q "whisper-server\|adamant-backend"; then
...
find_existing_databases() {
    local found_dbs=()
    local default_db_path="/opt/homebrew/Cellar/adamant-backend/0.0.4/backend/meeting_minutes.db"
...
start_server() {
    ...
    if docker images --format "{{.Repository}}:{{.Tag}}" | grep -q "adamant-backend:"; then
...
show_status() {
    ...
    if docker ps --format "{{.Names}}" | grep -q "adamant-backend"; then
        app_running=true
        local app_port=$(docker port adamant-backend 5167/tcp 2>/dev/null | cut -d: -f2)
        if [ -n "$app_port" ]; then
            log_info "Meeting App: http://localhost:$app_port"
            # Test connectivity
