...
$AppProjectName = "adamant-backend"
$AppContainerName = "adamant-backend"
$DefaultPort = 8178
$DefaultAppPort = 5167
...
function Invoke-StartCommand {
    ...
    $composeArgs += @("whisper-server", "adamant-backend")
    ...
    if ($DryRun) {
        ...
        if ($detach) {
            ...
            & docker-compose -f $ComposeFile --profile default up -d whisper-server adamant-backend
            ...
            & docker-compose -f $ComposeFile --profile default up whisper-server adamant-backend
...
function Invoke-LogsCommand {
    ...
    $serviceMap = @{
        ...
        "app" = "adamant-backend"
        "backend" = "adamant-backend"
        "meeting" = "adamant-backend"
    }
    ...
    if ($service) {
        ...
        $validServices = @("whisper-server", "adamant-backend")
        if ($service -notin $validServices) {
            Write-Warn "Unknown service: $service"
            Write-Info "Available services: whisper-server, adamant-backend"
        ...
        $logArgs += @("whisper-server", "adamant-backend")
    }
